...
def predict(net, raw_strokes):
    net.eval()
    size = 256
#     x = np.zeros((1, 3, size, size))
    x = np.zeros((1, 2, size, size))
    x[0] = draw_cv2(raw_strokes, size=size) / 255.0
    with torch.no_grad():
        ...
        maxk = max(topk)
        _, y_pred = outputs.topk(maxk, 1, True, True)
        top_1 = categorical_to_label(y_pred.cpu().data.numpy()[0][0])
        top_2 = categorical_to_label(y_pred.cpu().data.numpy()[0][1])
        top_3 = categorical_to_label(y_pred.cpu().data.numpy()[0][2])
    return top_1, top_2, top_3

# test on test set
test_df = pd.read_csv('test_simplified.csv')
all_strokes = test_df['drawing'].values

plt.figure(figsize=(20, 25))
for i, raw_strokes in enumerate(all_strokes):
    img = draw_cv2(raw_strokes, size=size) / 255.0
    plt.subplot(5, 5, i+1)
    plt.imshow(img[0], cmap='gray')
    preds = predict(net, raw_strokes)
    plt.title('[P1.{}]'.format(preds[0]) + '\n' + 
              '[P2.{}]'.format(preds[1]) + '\n' + 
              '[P3.{}]'.format(preds[2]) )
    if i >= 24:
        break
...
predictions_array = []
for i, raw_strokes in enumerate(tqdm(all_strokes[:])):
    predictions = predict(net, raw_strokes)
    predictions_array.append(predictions)

predictions_array = np.array(predictions_array)

words = [pred[0]+" "+pred[1]+" "+pred[2] for pred in predictions_array]
preds_df = pd.DataFrame({'key_id':key_ids, 'word':words})
preds_df.to_csv('test.csv')
preds_df.head()

classfiles = os.listdir('train_simplified/')
numstonames = {i: v[:-4].replace(" ", "_") for i, v in enumerate(classfiles)}
preds_df = pd.DataFrame({'first': predictions_array[:,0], 'second': predictions_array[:,1], 'third': predictions_array[:,2]})
# preds_df = preds_df.replace(numstonames)
preds_df['words'] = preds_df['first'] + " " + preds_df['second'] + " " + preds_df['third']
sub = pd.read_csv('sample_submission.csv', index_col=['key_id'])
sub['word'] = preds_df.words.values
# sub.to_csv('ResNeXt.csv')
sub.to_csv('ResNeXt_v2.csv')
sub.head()